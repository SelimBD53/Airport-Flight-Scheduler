...
class TravelAgent:
    ...
    """
     ...
     start_date: date when you to start the trip
    ...
    def set_trip_multi_city_flexible_route(self, trip_cities, start_date):
        ...
        for sequence in permutations(flexible_cities):
            ...
            fixed_route_trip = self.set_trip_multi_city_one_way_fixed_route(fixed_route,start_date)
            price = 0
            for trip in fixed_route_trip:
                price+=trip.price
            if price < min_price:
                min_price = price
                selected_trips = fixed_route_trip 
